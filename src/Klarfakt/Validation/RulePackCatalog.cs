...
/// <summary>
/// The rule packs Klarfakt validates against. The manifest is embedded in this assembly and names
...
public sealed class RulePackCatalog
{
    ...
    public static RulePackCatalog Load(string? rulesDirectory = null)
    {
        ...
        // other. KLARFAKT_RULES is routinely pasted from a shell or a container path.
    ...
    {
        if (Environment.GetEnvironmentVariable("KLARFAKT_RULES") is { Length: > 0 } configured)
        {
            return configured;
        }
