...
public class CorpusValidationTests(ValidatorFixture fixture)
{
    ...
    /// <summary>
    /// Published example invoices that Klarfakt reports errors on. Each was checked by hand
    /// against the rule expression and the document: all three are defects in the published
    /// examples, not disagreements in Klarfakt. KoSIT's own scenarios.xml runs the same two
